...
| C3 | Habilitations et cloisonnement des rôles | **77 %** | Cycle 3, **rôle caissier (addendum point h) vérifié au cycle 27** : diagnostic par exécution réelle, aucun code nécessaire — `qf_agent_comptabilite` remplit déjà les deux fonctions décidées le 2026-09-13 (encaisse ET saisit) : `POST /ventes` l'autorise déjà, `vente.html` est son écran d'accueil par défaut. **Correction d'une inexactitude du diagnostic d'origine** (même esprit que C6/C13) : le cloisonnement RH **est** déjà testé par une route (`GET /rh/employes` avec un jeton `agent_comptabilite` → 403, `server/tests/test_rh.py`), contrairement à ce que ce tableau affirmait. Fournisseurs : pas un vrai manque — la table n'a pas de `site_id` (partagée entre les deux sites par construction) et aucune route `/fournisseurs` n'existe (`404` vérifié). Habilitations appliquées **au niveau des requêtes SQL** : privilèges par colonne + RLS par site posés au cycle 2, exploités par `BaseDeDonnees.connexion_pour()`. Prouvé par exécution en **contournant l'API** : `SELECT ... WHERE site_id=2` sous `qf_agent_stock` renvoie 0 ligne même en le demandant explicitement (`server/tests/test_cloisonnement_site.py`). Reste ouvert : question 3 de l'addendum h (le caissier voit-il le détail des prix ou seulement le total ?, non tranchée), pas d'écran dédié à un profil « caissier » distinct de `vente.html`. **Cycle 35 (13b)** : RLS/GRANT révisés — catalogue commun aux deux sites (politique permissive sur `articles`), quantité cloisonnée sur `stocks_sites` (RLS par site, aucune écriture directe pour aucun rôle), comptage à l'aveugle avec `site_id` ; prouvé en SQL direct hors API (`test_cloisonnement_site.py`) et par `verifier-cablage.mjs` 94/94.  **Cycle 43 (C3, 2026-09-24)** : cumul de rôles décidé (addendum point h, question 4) et livré — migration 035 `utilisateurs_roles` (PK utilisateur+role, CHECK 3 rôles, reprise du rôle principal), `roles_utilisateur()` `SECURITY DEFINER`, jeton de session portant `roles`, `exiger_role` choisissant le rôle effectif par route (`SET ROLE` PostgreSQL), `POST /admin/comptes` acceptant `roles`, `GET /admin/comptes` exposant `roles`, écran `comptes.html` avec case « Cumuler aussi l'autre rôle agent » + affichage des rôles cumulés. Prouvé : pytest 229/229 (5 dédiés), suite SQL 000-035 + inverses verte, Playwright comptes 15/15 et câblage 94/94. Reste ouvert : question 3 du point h (détail des prix ou total seul pour le caissier — documentée EN ATTENTE, terrain vente réservé au point f). |
| C4 | Articles et stock | **89 %** | Cycles 9, 11 et 13. Six opérations réelles, chacune une fonction PostgreSQL `SECURITY DEFINER`, toutes câblées sur un vrai écran (`maquette/stock.html`, cycle 11). **Constats n°2 et n°3 corrigés (cycle 13)** : un retour client ou fournisseur ne peut plus dépasser, en article et en quantité (cumul de plusieurs retours compris), ce que la vente ou la réception d'origine porte réellement (migration 016) ; `PUT /articles/{id}` ne trace plus de changement de prix dans `historique_prix_articles` quand le prix soumis est identique à l'actuel. `quantite_stock` volontairement jamais modifiable par fiche. **Aucun montant FCFA, aucun champ de prix n'atteint la page ni les réponses réseau de l'agent stock**. Vérifié par exécution : 104/104 tests pytest (100 + 4 nouveaux), suite SQL à jour (44/44, 52/52, 6/6, réversibilité de la migration 016 confirmée, vérifiée en SQL direct avant tout code Python), 6 suites Playwright — 0 régression (aucun écran touché par la correction). Manquent : volumétrie/reprise du stock initial (addendum j, non tranché côté code — décidé le 2026-09-19, débloqué par le cycle 35), export dédié à C4, remises et article offert (point f, sous-chantiers 3-4, non construits). **Cycle 35 (13b)** : modèle multi-site livré — `enregistrer_entree_stock` ouvre la ligne du site (première réception), `transferer_stock` = UN article + deux sites (ligne de destination créée), casse/retours sur (article, site), `annuler_vente` restitue par site ; écran `stock.html` adapté ; `verifier-stock-reel.mjs` 26/26, `test_stock.py` 16/16. **Cycle 39 (point f, sous-chantier 1, 2026-09-22)** : quantités décimales par article — `articles.quantite_decimale_autorisee`, les 6 fonctions de stock réécrites en `NUMERIC(12,3)` (mêmes règles, mêmes verrous), `GET/POST/PUT /articles` exposent et acceptent le nouveau champ. **Prouvé par exécution** : suite SQL 000-034 + inverses verte (5 contrôles dédiés : refus/acceptation/non-arrondi), pytest 224/224, `verifier-cablage.mjs` 94/94. **Cycle 40 (point f, sous-chantier 2, 2026-09-23)** : retours enrichis — casse et retour client passent en deux temps (déclaration ouverte à tout rôle qui touche au stock, sans effet sur le stock ; validation réservée au responsable, qui décrémente/réintègre réellement) ; retour client avec 3 issues (échange/avoir/remboursement espèces — ce dernier exige une confirmation explicite et crée une dépense) et état de la marchandise (revendable réintégré, invendable tracé sans mouvement fantôme — bug de cumul trouvé et corrigé par exécution : un retour invendable validé n'était pas compté dans le total « déjà rendu », permettant un dépassement silencieux du vendu). Anciennes fonctions à un seul temps supprimées. **Prouvé par exécution** : suite SQL 000-034,036 + inverses verte (protections 71/71, habilitations 53/53), pytest 229/229, `verifier-cablage.mjs` 94/94. **Cycle 46 (point f, sous-chantier 4, 2026-09-24)** : article offert — sortie commerciale gratuite distincte d'une remise à 100 % (migration 037, qui la refuse), même schéma déclaration/validation que la casse (ouverte à qui vend, agent comptabilité/responsable ; validation réservée au responsable). `employe_id` obligatoire (fiche RH, actif, même site — exigence du 2026-09-24 : « le geste le plus facile à détourner »), distinct de `declarant_id` et de `ventes.vendeur_id` (compte utilisateur, inchangé). `vente_id` nullable, `valeur_normale` figée depuis le prix catalogue. Écart de temps déclaration/validation traité : `GET /inventaire/ecarts` signale les déclarations en attente sur le même article, nouvelle carte tableau de bord « Articles offerts en attente de validation » avec ancienneté. Deux bugs de contrainte trouvés et corrigés par exécution avant tout commit (`chk_mouvements_categorie*` avait perdu `annulation_vente`, `chk_mouvements_vente_id_coherent` refusait `vente_id` pour cette nouvelle catégorie). **Prouvé par exécution** : suite SQL 000-039 + inverses verte (protections 103/103 dont 14 dédiés, habilitations 53/53), pytest 258/258, `verifier-cablage.mjs` 94/94, vérification visuelle réelle dans Chrome. **Reste (point f)** : écran de déclaration/validation dédié non câblé sur `stock.html` (listing/validation des trois types de déclarations — casse, retour client, article offert — reste à faire, comme prévu depuis le sous-chantier 2) ; case décimale sur la fiche article. Le plan des 4 sous-chantiers du point f est désormais entièrement livré. |
| C5 | Ventes et facturation | **75 %** | Cycle 6, durci par le cycle de correction après C7, complété par les cycles 17, 19 et **27** (addendum point c, décidé le 2026-09-13). **Cycle 27, périmètre volontairement réduit au socle décidé** (`db/migrations/020_facturier_vendeur.sql`) : `numero_facturier` (référence du carnet PAPIER, transcrite par le comptable — jamais générée par le logiciel, préfixe `MAG-`/`CPT-` vérifié par site) et `vendeur_id` (qui a négocié le prix, distinct de qui saisit/encaisse) obligatoires sur toute nouvelle vente ; nouvelle route `GET /ventes/vendeurs` (comptes actifs du site + responsable) pour le menu déroulant de l'écran ; affiché sur le reçu PDF et dans l'export `/rapports/ventes`. **PAS construits ce cycle** (questions non tranchées à l'époque) : rapport « écarts de prix par vendeur » et son seuil de validation (question 5), liste de vendeurs sans compte (question 3). **Question 5 tranchée le 2026-09-18** : seuil de 10 % sous le prix catalogue, en configuration, signale sans jamais bloquer (voir `ADDENDUM_CAHIER_DES_CHARGES.md`, point c) — **le rapport lui-même reste à construire** (chantier non démarré), toujours retenu par la question 3 (vendeur sans compte), encore ouverte. Décisions antérieures (points b/d/e) inchangées : régime réel, TVA 19,25 % sur prix TTC, une vente déjà encaissée n'est jamais bloquée, crédit client désactivé. `POST /ventes/{id}/annuler` (cycle 17), `GET /ventes/{id}/recu` (cycle 19, PDF). **Cycle 42 (point f, sous-chantier 3, 2026-09-23)** : remises — par ligne (`prix_catalogue`/`remise_montant`, cohérence vérifiée par une `CHECK` — `GREATEST(catalogue - payé, 0)`, upsell toléré) et de la vente entière (`remise_globale_montant`), deux mécanismes séparés ; `POST /ventes` accepte au choix `prix_unitaire`, `remise_montant` ou `remise_pct` par ligne (exactement un des trois) ; remise à 100 % refusée (réservée au sous-chantier 4, article offert) ; seuil de validation responsable prêt mais inactif (`seuil_remise_validation_pct`, sentinelle `a_definir`) ; reçu PDF affiche catalogue/remise/payé quand une remise est présente. Vérifié par exécution : 33/33 tests pytest dédiés à C5 (172/172 au total, 9 nouveaux pour le point c) **puis 239/239 après le cycle 42 (10 nouveaux dédiés aux remises)**, suite SQL à jour (79/79 protections dont 8 dédiés à la migration 037, 53/53, 6/6, réversibilité confirmée), 12/12 + 94/94 + 27/27 contrôles Playwright (vente, câblage, corrections UX). Manquent : rapport d'écarts de prix par vendeur (seuil désormais connu, question 3 encore ouverte), vendeurs sans compte, ticket thermique/code-barres (non demandés), impression physique sur une imprimante réelle (non vérifiable par l'agent), écran non câblé pour les remises (saisie sur `vente.html`). |
...
| C11 | Sécurité applicative | **85 %** | Cycle 3, complété au cycle 22. Le « Sécurité : 100 % » du diagnostic d'origine était un artefact (mots-clés trouvés dans le script de diagnostic lui-même) — désormais vérifié réellement : démarrage refuse `postgres` et toute clé d'exemple, requêtes systématiquement paramétrées (injection SQL testée), jetons signés HMAC vérifiés à temps constant, aucun hachage ne fuit dans aucune réponse, erreurs SQL jamais renvoyées telles quelles au client. **Cycle 22** : révocation de session (migration 018, `POST /auth/deconnexion`, `deps.obtenir_session()` vérifie la révocation à chaque requête — un jeton révoqué signature-valide et non expiré est quand même refusé, prouvé en le rejouant après déconnexion) ; en-têtes HTTP de sécurité (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`) sur toute réponse. Vérifié par exécution : 4/4 tests pytest dédiés (144/144 au total, 0 régression, coût mesuré ~13 % de temps d'exécution en plus), `verifier-cablage.mjs` +3 (80/80, jeton intercepté avant déconnexion puis rejoué → refusé). Reste : limiteur de débit partagé entre processus (délibérément pas fait ce cycle — mérite sa propre vérification, pas glissé à la suite de deux autres changements de sécurité), audit plus large (dépendances, TLS — hors périmètre local de dev).  **Cycle 44 (C11, 2026-09-24)** : limiteur de débit PARTAGÉ entre processus — migration 038 `limitations_debit` + `tentative_autorisee()` (upsert atomique + `FOR UPDATE`, fenêtre glissante, `SECURITY DEFINER`) + `reinitialiser_limitation()`, `GRANT EXECUTE` à `qf_app` ; la connexion appelle la base à chaque tentative (repli en mémoire si la base ne répond pas), réinitialise le compteur après succès ; seuil 10 tentatives/minute INCHANGÉ (décision propriétaire). Prouvé : pytest 247/248 (4 dédiés, 1 flaky test_caisse non lié — passe isolément), suite SQL 000-038 + inverses verte, Playwright câblage 94/94. Reste : audit de dépendances/TLS (documenté comme limite, hors périmètre local).  **Cycle 47 (C11, 2026-09-24)** : audit de dépendances/TLS — `pip-audit` sur le lockfile : `python-multipart 0.0.20 → 0.0.31` (corrigé + lockfile régénéré + test_configuration 11/11) ; starlette 0.48.0 (fix 1.1.0, couplé FastAPI) et pytest 8.3.5 (fix 9.0.3, outil de test) documentés comme risques acceptés (décision propriétaire) avec plan de traitement après fusion du point f ; posture TLS documentée (HTTP local, hachage bcrypt en base, TLS à la passerelle du tunnel si un jour utilisé). Rapport : `RAPPORT AVANCEMENT/audit-dependances.md`.  **Cycle 49 (C11, 2026-09-25)** : montée majeure décidée — fastapi 0.117.1 → **0.141.1**, starlette 0.48.0 → **1.7.0**, pytest 8.3.5 → **9.1.1** (dernières disponibles), pydantic 2.13.5 ; validée dans un venv séparé (`server/.venv-upgrade`) sans aucune modification de code — pytest **261/261**, Playwright câblage **94/94**, `pip-audit` **« No known vulnerabilities found »** ; `requirements.in` épingle starlette en direct (fastapi la laisse libre) + lockfile régénéré avec hachages. Risques starlette/pytest du rapport d'audit : **traités**. |
...
| C13 | Tests automatisés et qualité | **68 %** | Cycle 20, complété au **cycle 27** : **172 tests pytest**, une **suite SQL complète** (44+52+6 contrôles + réversibilité) et **9 suites Playwright** (261 contrôles) existent et sont rejouées à chaque cycle qui touche le code correspondant. `db/outils/verifier_tout.sh` enchaîne les trois couches en une seule commande. **Cycle 27** : un des deux points bloquant une CI Linux, cité depuis le cycle 20, est corrigé — `server/tests/conftest.py` cherchait `psql.exe` en dur dans `_pgdev/pgsql/bin/` (distribution portable Windows, absente de tout runner CI) ; cherche désormais `psql` sur le `PATH` en premier (`shutil.which`), ne retombant sur le chemin Windows que s'il est introuvable — comportement du poste de développement inchangé (vérifié : aucun `psql` sur son `PATH`, le repli s'active toujours), 172/172 tests toujours au vert. Reste, non traité ce cycle : `PGHOST`/`PGPORT` (5433, instance portable) et le mot de passe de test en dur restent, eux aussi, propres à ce poste — un runner CI aurait besoin de ses propres variables d'environnement ; aucun pipeline CI (fichier de workflow) n'existe encore. Couverture des parcours nécessitant une imprimante ou un téléphone réels toujours hors de portée d'une suite automatisée.  **Cycle 48 (C13, 2026-09-24)** : le seul test flaky constaté (`test_cloture_ne_se_supprime_pas_meme_en_sql_direct`) est marqué `@pytest.mark.flaky(reruns=2, reruns_delay=5)` — reprise automatique en cas d'échec transitoire, sans masquer un échec déterministe ; `pytest.ini` enregistre le marqueur ; `pytest-rerunfailures==16.0` ajouté au lockfile avec hachages. Prouvé : lock 37 paquets / 0 sans hachage, test_caisse 19/19. Aucune autre suite de test touchée.  **Cycle 52 (C13, 2026-09-25)** : chasse aux trous de couverture — `verifier-stock-reel.mjs` RÉÉCRIT (il testait encore la casse et le retour client à un seul temps et affirmait que l'agent stock n'a pas de bouton Casse — contredisait le point f/chantier A, suite rouge invisible car la CI n'exécute que pytest) : 38/38 dont déclarations sans effet stock + validation sur `declarations.html` (casse décrémente, retour revendable réintègre, offert décrémente) ; nouveau `verifier-exploitation-reel.mjs` (12/12) : voyant sauvegarde par la VRAIE route `/exploitation/derniere-sauvegarde` (le câblage mimait le fichier), carte Ventes du jour, carte Articles offerts, logo téléversé/affiché/supprimé depuis l'écran, 403 agent stock ; ajouté à `verifier_tout.sh` et `package.json` ; `verifier-routes-appelees.mjs` 55/55. |
| C14 | Documentation et livrables | **65 %** | Évalué sur pièces. Documentation d'usage/recette solide : CDC détaillé, 2 guides testeur, dossier de recette, guide d'installation. `MODELE_DONNEES.md`, `PERIMETRE_LIVRE.md`, `ADDENDUM_CAHIER_DES_CHARGES.md` produits dans ce cycle. Manquent (CDC §7) : code source, scripts de fabrication des exécutables, scripts + guide de sauvegarde/restauration.  **Cycle 45 (C14, 2026-09-24)** : livrables CDC §7 tous présents et documentés — code source (dépôt Git, `OWNERSHIP.md`), schéma + migrations 000-038 (+ inverses), scripts de fabrication (`server/fabrication/construire.ps1` + `.spec` PyInstaller), sauvegarde/restauration (`db/outils` + guide, RPO 1 h / RTO 2 h / AES-256), jeux de tests (pytest 248, suite SQL, Playwright), CI GitHub Actions, documentation d'installation et d'exploitation ; `README.md` racine remis à jour (état cycle 44 + liste des livrables §7 + résumé opérationnel). Reste : étiqueter une version (tag) lors de la première livraison réelle, clause contractuelle d'escrow (hors périmètre technique, point l).  **Cycle 50 (C14, 2026-09-25)** : premier tag de version **`v0.1.0`** posé et poussé sur `main` (commit de la montée majeure C11, moyenne ≈ 74 %). Reste : clause contractuelle d'escrow (hors périmètre technique, point l). |

**Moyenne indicative après le cycle 55 (C1 normalisation des noms) : ≈ 76 %** (C0 78, C1 92, C2 77, C3 77, C4 89, C5 72, C6 85, C7 65, C8 72, C9 60, C10 58, C11 85, C12 92, C13 68, C14 65).
...
### Cycle 40 — C1 : preuve de reprise sur volume réel — 2026-09-22

- **Décisions du propriétaire** (2026-09-22) : RTO **2 heures** ; chiffrement
  de la copie distante et notification d'échec **documentés comme limites** ;
  volume de preuve **généré** (~1 000 références, 30 jours d'historique).
- **Livré** : `db/outils/generer_volume_realiste.ps1` + `db/tests/volume_realiste.sql`
  (reconstruit `quincaillerie_volume` de zéro : schéma d'origine + migrations
  000-033 + 1 000 articles, 2 000 stocks, 1 996 mouvements, 1 200 ventes,
  3 600 lignes de vente, 120 comptages, 3 comptes) ; guide de sauvegarde mis
  à jour (RTO 2 h, outil, limites) ; arbre `db/README.md` complété.
- **Preuve par exécution** : `sauvegarder.ps1` → dump **308,8 Ko chiffré
  AES-256** + rôles ; `restaurer.ps1` → **28 tables** restaurées sur
  `quincaillerie_restore` ; comptages **identiques** sur 8 tables ; **10
  dernières ventes** et **soldes par site** identiques ; API reprise sur la
  base restaurée : `/sante` 200, connexion `resp.volume` 200,
  `GET /admin/comptes` 200 (3 comptes).
- **Score** : C1 **87 % → 90 %** (après le point f du cycle 39). Moyenne
  indicative ≈ 71 %.
- **Reste (C1)** : reprise sur les données réelles du propriétaire au moment
  du démarrage (point j de l'addendum — import du stock initial).

---

### Cycle 41 — C4, point f sous-chantier 2 : retours enrichis — 2026-09-23

*(Numérotée 41, pas 40 : ce numéro de cycle a été pris entretemps par le
travail C1 ci-dessus, comme la migration 035 l'a été par un autre
chantier en parallèle — voir plus bas.)*

- **Décisions du propriétaire** (ADDENDUM_CAHIER_DES_CHARGES.md, point f,
  réponses aux questions 1 et 2, 2026-09-22 ; plan validé le 2026-09-23) :
  casse — déclaration ouverte à tout rôle qui touche au stock (simple
  constat), validation réservée au responsable (décrémente réellement) ;
  retour client — validation du responsable obligatoire, trois issues
  (échange / avoir client / remboursement espèces — confirmation explicite
  supplémentaire exigée pour ce dernier), état de la marchandise vérifié
  avant toute réintégration. Marchandise invendable : tracée sans nouvelle
  catégorie `mouvements_stock` (précision technique, pas une décision
  métier — voir ci-dessous) ; pas de route d'historique de vente (hors
  périmètre) ; un responsable peut valider sa propre déclaration de casse.
- **Livré** : migration **036** (+ inverse) — renumérotée depuis 035, pris
  entretemps par un autre chantier en parallèle (C3, cumul de rôles),
  reconfirmé juste avant l'écriture puis juste avant la fusion.
  `declarations_casse`/`declarations_retour_client` (RLS par site) ;
  `declarer_casse()`/`valider_casse()`,
  `declarer_retour_client()`/`valider_retour_client()` (`SECURITY
  DEFINER`, validation réservée à `qf_responsable`) ; anciennes fonctions
  à un seul temps supprimées (une seule voie d'écriture). API :
  `POST/GET /stock/casse/declarations`,
  `POST /stock/casse/declarations/{id}/valider`,
  `POST/GET /stock/retours-client/declarations`,
  `POST /stock/retours-client/declarations/{id}/valider`.
- **Précision technique trouvée en concevant la fonction** (pas une
  décision métier) : un retour invendable ne peut **pas** générer de
  mouvement `mouvements_stock` réel, même de catégorie `casse` — un tel
  mouvement décrémenterait `stocks_sites`, or l'article n'a jamais été
  réintégré. La perte est tracée uniquement dans
  `declarations_retour_client` (`etat_marchandise='invendable'`), imposé
  par une contrainte `CHECK` (pas seulement par discipline applicative).
- **Bug trouvé par exécution et corrigé avant la fusion** : le cumul
  « déjà rendu », qui bloque un dépassement du vendu, ne comptait que
  `mouvements_stock` — qui ne voit **jamais** les retours invendables
  (aucun mouvement). Un retour pouvait donc dépasser silencieusement la
  quantité vendue (ex. 3 échangés + 2 remboursés + 1 invendable = 6
  réellement rendus, mais `mouvements_stock` n'en voyait que 5). Source de
  vérité changée pour `declarations_retour_client` (`statut='validee'`),
  qui voit les deux cas — reproduit puis vérifié corrigé par exécution
  réelle avant tout commit.
- **Vérifié par exécution réelle**, sur une base et un serveur dédiés
  (`quincaillerie_point_f`, port 8020, worktree
  `_worktrees/piste-point-f`) : suite SQL **000-034,036 + inverses**
  verte (protections **71/71**, habilitations **53/53**, concurrence,
  réversibilité — refuse bruyamment si des déclarations réelles existent,
  comme la migration 034) ; pytest complet **229/229**, 0 régression ;
  `verifier-cablage.mjs` **94/94**.
- **Score** : C4 **83 % → 86 %**. Moyenne indicative ≈ 71 %.
- **Reste (point f)** : écran non câblé (listing/validation des
  déclarations, case décimale sur la fiche article) ; sous-chantiers 3-4
  du plan (remises, article offert), non commencés.

---

### Cycle 42 — C5, point f sous-chantier 3 : remises — 2026-09-23

- **Décisions du propriétaire** (ADDENDUM_CAHIER_DES_CHARGES.md, point f,
  réponse à la question 4, 2026-09-22 ; plan validé le 2026-09-23) :
  remise toujours visible explicitement sur le ticket (catalogue, remise,
  payé — jamais une modification silencieuse du prix affiché) ; applicable
  au choix par ligne ou sur la vente entière, deux mécanismes séparés,
  jamais une distribution automatique entre lignes ; montant OU
  pourcentage au choix du vendeur ; les trois valeurs conservées
  séparément ; autorisation du responsable exigée au-delà d'un seuil
  (`a_definir`, mécanique prête, aucun blocage tant que non fixé — distinct
  du seuil du point c, question 5, usages différents). **Précision
  complémentaire tranchée à la validation du plan (2026-09-23)** : une
  remise à 100 % est refusée ici — c'est le sous-chantier 4 (article
  offert) qui la couvre, un seul chemin d'écriture par concept.
- **Livré** : migration **037** (+ inverse) — 035 pris entretemps par un
  autre chantier en parallèle (C3, cumul de rôles), 036 par le sous-chantier
  2 ci-dessus, reconfirmé juste avant l'écriture puis juste avant la
  fusion. `ventes_lignes.prix_catalogue` (nullable, tolérance legacy) et
  `remise_montant` (`NOT NULL DEFAULT 0`, cohérence vérifiée par
  `chk_ventes_lignes_remise_coherente` — `GREATEST(catalogue - payé, 0)`,
  pas une égalité stricte, pour tolérer un prix négocié au-dessus du
  catalogue sans le traiter comme une remise négative) ;
  `ventes.remise_globale_montant` (`chk_ventes_remise_globale_positive`) ;
  `chk_ventes_lignes_prix_positif` resserrée à `> 0` (refuse la remise à
  100 %) ; paramètre `seuil_remise_validation_pct` (sentinelle
  `a_definir`). `POST /ventes` : chaque ligne accepte au choix
  `prix_unitaire`, `remise_montant` ou `remise_pct` (exactement un des
  trois, `model_validator` Pydantic) ; la vente accepte au choix
  `remise_globale_montant` ou `remise_globale_pct` (au plus un des deux,
  refusée si elle atteint ou dépasse le total) ; reçu PDF étendu (colonnes
  catalogue/remise/payé + ligne « Remise sur la vente », seulement quand
  une remise est réellement présente — inchangé sinon).
- **Vérifié par exécution réelle**, sur une base et un serveur dédiés
  (`quincaillerie_point_f`, port 8014, worktree `_worktrees/piste-point-f`) :
  suite SQL **000-034,036,037 + inverses** verte (protections **79/79**,
  dont 8 contrôles dédiés à la migration 037 — cohérence, refus 100 %,
  upsell toléré, tolérance legacy, remise globale positive/négative ;
  habilitations **53/53** ; concurrence **6/6** ; réversibilité — refuse
  bruyamment si des remises réelles existent) ; pytest complet **239/239**
  (229 + 10 nouveaux dédiés aux remises dans `test_ventes.py`), 0
  régression ; `verifier-cablage.mjs` **94/94**.
- **Score** : C5 **72 % → 75 %**.
- **Reste (point f)** : sous-chantier 4 du plan (article offert), non
  commencé ; écran non câblé pour la saisie d'une remise (`vente.html`
  n'expose aujourd'hui que le prix négocié direct, déjà câblé).

---

### Cycle 43 — C3 : cumul de rôles sur un même compte — 2026-09-24

- **Décisions du propriétaire** (2026-09-22) : cumul de rôles **OUI**
  (addendum point h, question 4, petit effectif) ; question 3 du point h
  (détail des prix ou total seul pour le caissier) **documentée EN ATTENTE**
  — terrain vente réservé au point f en cours.
- **Livré** : migration 035 (+ inverse) — `utilisateurs_roles`
  (PK utilisateur+role, CHECK 3 rôles, FK CASCADE, reprise du rôle
  principal), `roles_utilisateur()` `SECURITY DEFINER` (fenêtre minimale
  `qf_app`/`qf_responsable`), GRANT SELECT/INSERT au responsable ; jeton de
  session portant `roles` (compatibilité jetons antérieurs) ;
  `Session.roles` + `avec_role()` ; `exiger_role` choisit le rôle effectif
  par route pour le `SET ROLE` PostgreSQL — aucune route existante modifiée ;
  `POST /admin/comptes` accepte `roles`, `GET /admin/comptes` expose
  `roles` ; écran `comptes.html` : case « Cumuler aussi l'autre rôle agent »
  + affichage « Agent stock + Agent comptabilité ».
- **Preuves** : pytest **229/229** (5 tests dédiés : jeton portant les rôles,
  accès aux deux périmètres avec un seul compte, refus hors rôles, agent seul
  refusé sur le périmètre compta, rôle principal hors liste refusé) ; suite
  SQL **000-035 + inverses** verte (4 contrôles) ; Playwright
  `verifier-comptes-reel.mjs` **15/15** et `verifier-cablage.mjs` **94/94**.
- **Score** : C3 **72 % → 77 %**. Moyenne indicative ≈ 71 %.
- **Reste (C3)** : question 3 du point h (en attente de terrain vente libre).

---

### Cycle 44 — C11 : limiteur de débit partagé entre processus — 2026-09-24

- **Décisions du propriétaire** (2026-09-24) : seuil **10 tentatives/minute
  inchangé** ; audit de dépendances/TLS **documenté comme limite**.
- **Livré** : migration 038 (+ inverse) — `limitations_debit` +
  `tentative_autorisee()` (upsert atomique + `FOR UPDATE`, fenêtre
  glissante, `SECURITY DEFINER`) + `reinitialiser_limitation()`,
  `GRANT EXECUTE` à `qf_app` ; `auth.py` : la connexion appelle la base à
  chaque tentative (repli en mémoire si la base ne répond pas — la
  limitation ne bloque jamais la connexion), réinitialise le compteur après
  succès ; `LimiteurDebit` expose `max_essais`/`fenetre_secondes`.
- **Preuves** : pytest **247/248** (4 tests dédiés : compteur partagé entre
  deux connexions distinctes, route 429 après le seuil puis 401 après
  réinitialisation, clés indépendantes, connexion normale ; 1 échec flaky
  `test_caisse` non lié — passe isolément) ; suite SQL **000-038 + inverses**
  verte (5 contrôles) ; Playwright `verifier-cablage.mjs` **94/94**.
- **Score** : C11 **68 % → 73 %**. Moyenne indicative ≈ 71 %.
- **Reste (C11)** : audit de dépendances/TLS (limite documentée).

---

### Cycle 45 — C14 : livrables CDC §7 documentés, README remis à jour — 2026-09-24

- **Diagnostic** : la ligne C14 indiquait « Manquent (CDC §7) : code source,
  scripts de fabrication, scripts + guide de sauvegarde/restauration » — tous
  **existent désormais** (dépôt Git + `OWNERSHIP.md`, `server/fabrication/`,
  `db/outils` + guide). Le `README.md` racine, lui, était resté figé au
  cycle 4 (migrations « 9 », tests « 36/36 », lanceur « pointe sur /docs »).
- **Livré** : `README.md` entièrement remis à jour — état réel (cycle 44,
  moyenne ≈ 71 %), **liste des livrables CDC §7** avec chemin de chaque
  exigence (code, schéma+migrations 000-038, build PyInstaller,
  sauvegarde/restauration RPO 1 h/RTO 2 h/AES-256, jeux de tests, CI,
  documentation), architecture du dépôt, résumé opérationnel
  (reconstruire/vérifier), contexte métier à jour (cumul de rôles cycle 43).
- **Score** : C14 **40 % → 60 %**. Moyenne indicative ≈ 73 %.
- **Reste (C14)** : étiqueter une version (tag) lors de la première livraison
  réelle ; clause contractuelle d'escrow (hors périmètre technique, point l).

---

### Cycle 46 — C4, point f sous-chantier 4 : article offert — 2026-09-24

*(Numérotée 46, pas 45 : ce numéro a été pris entretemps par le travail C14
ci-dessus — même situation que les cycles 41 et migrations 035/036/038
documentées plus haut.)*

- **Décisions du propriétaire** (ADDENDUM_CAHIER_DES_CHARGES.md, point f,
  réponse à la question 5, 2026-09-22 ; précisions du 2026-09-24, validation
  du plan) : un article offert est une sortie commerciale gratuite
  **distincte** d'une remise à 100 % (migration 037, qui la refuse
  explicitement) — jamais une vente à prix nul, l'article est bien déduit du
  stock. Même schéma déclaration → validation que la casse (sous-chantier
  2) : ouverte à qui vend (agent comptabilité, responsable, **pas** l'agent
  stock) ; validation réservée au responsable, qui décrémente réellement.
  **Exigence complémentaire (2026-09-24)** : « un article offert est le
  geste le plus facile à détourner » — `employe_id` **obligatoire**,
  référence la fiche RH (`employes`, actif, même site), distinct de
  `declarant_id` (qui saisit) et de `ventes.vendeur_id` (compte utilisateur,
  migration 020 — la question 3 de l'addendum point c reste ouverte,
  **volontairement non touchée** par ce cycle : deux notions distinctes
  coexistent, rien ne les unifie). `vente_id` **nullable** : un article
  offert peut accompagner un achat réel (« 10 achetés, 1 offert ») ou être
  une opération autonome. `valeur_normale` figée à la déclaration depuis
  `articles.prix_vente`, jamais relue après coup.
- **Écart de temps déclaration → validation traité** (décision 2026-09-24) :
  tant que non validée, la marchandise est physiquement partie mais le
  stock système ne bouge pas. `GET /inventaire/ecarts` renvoie désormais
  `declarations_offert_en_attente` par ligne (signale un cadeau non validé
  plutôt qu'un vol) ; nouvelle carte tableau de bord « Articles offerts en
  attente de validation » (`GET /tableau-bord/articles-offerts-en-attente`),
  ancienneté calculée côté écran.
- **Livré** : migration **039** (+ inverse) — `declarations_article_offert`
  (RLS par site) ; catégorie `article_offert` sur `mouvements_stock` ;
  `declarer_article_offert()`/`valider_article_offert()` (`SECURITY
  DEFINER`). API : `POST/GET /stock/articles-offerts/declarations`,
  `POST /stock/articles-offerts/declarations/{id}/valider`.
- **Deux bugs de contrainte trouvés et corrigés par exécution, avant tout
  commit** : `chk_mouvements_categorie`/`..._type_coherent`, réécrites en
  reprenant la migration 014 (qui ne connaissait pas encore
  `annulation_vente`, ajoutée par la 017), faisaient régresser
  `test_annulation_vente_regularise_automatiquement_lecart` — corrigé en
  reprenant la liste de la 017, reconfirmé par un test SQL dédié.
  `chk_mouvements_vente_id_coherent` (migration 017) refusait `vente_id`
  pour toute catégorie hors vente/retour_client/annulation_vente — élargie
  à `article_offert` (autorisé, jamais obligatoire).
- **Bug d'affichage trouvé et corrigé par capture d'écran réelle** : la note
  d'alerte ajoutée à la carte « Écarts d'inventaire » écrasait le nom de
  l'article (répartition flexbox entre trop d'enfants directs de `<li>`) —
  déplacée dans la bonne colonne (`spanPrincipal`).
- **Vérifié par exécution réelle**, sur une base et un serveur dédiés
  (`quincaillerie_point_f`, port 8015, worktree `_worktrees/piste-point-f`) :
  suite SQL **000-039 + inverses** verte (protections **103/103**, dont 14
  contrôles dédiés à la migration 039 ; habilitations **53/53** ;
  concurrence ; réversibilité — refuse bruyamment si des déclarations
  réelles existent) ; pytest complet **258/258** (10 nouveaux dédiés à
  l'article offert) ; `verifier-cablage.mjs` **94/94** ; vérification
  visuelle réelle dans Chrome (déclaration créée via l'API, comptage
  déclenchant un écart, capture d'écran des deux nouvelles cartes).
- **Score** : C4 **86 % → 89 %**.
- **Reste (point f)** : le plan des 4 sous-chantiers est désormais
  entièrement livré (quantités décimales, retours enrichis, remises,
  article offert). Reste hors de ce plan : écran de déclaration/validation
  dédié sur `stock.html` pour les trois types de déclarations (casse,
  retour client, article offert — listing + bouton de validation, jamais
  câblé depuis le sous-chantier 2) ; case décimale sur la fiche article.
- **Note annexe** : fermeture de la PR #47 (sous-chantier 2, retours
  enrichis) sans fusion — son contenu était déjà intégralement sur `main`
  (commit `718da3f`, fusionné directement) ; la fusionner telle quelle
  aurait supprimé le cumul de rôles et les remises arrivés depuis. Branche
  distante supprimée.
### Cycle 47 — C11 : audit de dépendances et de TLS — 2026-09-24

- **Décision du propriétaire** : starlette/pytest documentés comme **risques
  acceptés** (pas de saut de version majeur pendant le point f).
- **Livré** : `python-multipart 0.0.20 → 0.0.31` (PYSEC-2026-1852/3036/3037/
  3038/3039/3040 corrigés) + `requirements.in` recréé + lockfile
  `requirements.txt` régénéré avec hachages ; rapport
  [`RAPPORT AVANCEMENT/audit-dependances.md`](audit-dependances.md) —
  starlette 0.48.0 et pytest 8.3.5 consignés avec impact et plan de
  traitement après fusion du point f ; posture TLS documentée (HTTP local,
  bcrypt en base, TLS à la passerelle du tunnel si un jour utilisé).
- **Preuves** : `pip-audit` ne signale plus `python-multipart` ;
  `pytest server/tests/test_configuration.py` **11/11**.
- **Score** : C11 **73 % → 78 %**. Moyenne indicative ≈ 73 %.

---

### Cycle 48 — C13 : reprise automatique du seul test flaky — 2026-09-24

- **Diagnostic** : `test_cloture_ne_se_supprime_pas_meme_en_sql_direct`
  (`server/tests/test_caisse.py:197`) a échoué **une fois en suite complète**
  (contention de base pendant le travail parallèle) alors qu'il passe
  isolément — flaky, pas déterministe.
- **Livré** : marqueur `@pytest.mark.flaky(reruns=2, reruns_delay=5)` posé
  UNIQUEMENT sur ce test (reprise après 5 s, jamais de masquage d'un échec
  déterministe qui resterait rouge) ; `pytest.ini` enregistre le marqueur ;
  `pytest-rerunfailures==16.0` ajouté à `requirements.in` et au lockfile
  `requirements.txt` (hachages). **Aucune autre suite de test touchée.**
- **Preuves** : lock **37 paquets / 0 sans hachage** ;
  `pytest server/tests/test_caisse.py` **19/19**.
- **Score** : C13 **60 % → 63 %**. Moyenne indicative ≈ 74 %.

---

### Cycle 49 — C11 : montée majeure FastAPI/starlette/pytest — 2026-09-25

- **Décision du propriétaire** : dernières versions disponibles
  (`fastapi 0.141.1`, `starlette 1.7.0`, `pytest 9.1.1`).
- **Livré** : `requirements.in` mis à jour (starlette épinglée en direct :
  fastapi la laisse libre et le résolveur reprenait l'ancienne 0.48.0) +
  lockfile `requirements.txt` régénéré avec hachages ; **aucun changement
  de code applicatif nécessaire**.
- **Preuves (venv séparé `server/.venv-upgrade`, sans déranger le venv
  partagé)** : pytest complet **261/261** ; Playwright
  `verifier-cablage.mjs` **94/94** ; `pip-audit` :
  **« No known vulnerabilities found »** — les deux risques acceptés du
  cycle 47 (starlette, pytest) sont **traités**.
- **Score** : C11 **78 % → 85 %**. Moyenne indicative ≈ 74 %.

---

### Cycle 50 — C14 : premier tag de version v0.1.0 — 2026-09-25

- **Décision du propriétaire** : `v0.1.0` — version de reconstruction, pas
  encore de livraison réelle en boutique.
- **Livré** : tag annoté **`v0.1.0`** posé et poussé sur `main` (commit de
  la montée majeure C11, moyenne ≈ 74 %).
- **Score** : C14 **60 % → 65 %**. Moyenne indicative ≈ 74 %.
- **Reste (C14)** : clause contractuelle d'escrow (hors périmètre
  technique, point l).

---

### Cycle 51 — C7 : plafond de vraisemblance DÉCIDÉ à 10 000 — 2026-09-25

- **Correction (2026-09-25)** : la valeur 10 000 est une **valeur par
  défaut posée par l'équipe de développement**, EN ATTENTE DE
  CONFIRMATION du propriétaire (le paramètre était marqué à
  trancher par lui). La valeur n'est PAS modifiée ; la référence
  est corrigée par la migration **042** (+ inverse) et le point
  est ajouté à la liste des confirmations à obtenir.
- **Livré** : migration **040** (+ inverse) — `plafond_vraisemblance_comptage
  = 10000`, `a_decider = FALSE` ; jeu d'essai synchronisé ; tests
  pytest réécrits (10 001 refusé / 10 000 accepté / paramètre
  re-décidable) ; 3 contrôles SQL (section 13).
- **Preuves** : suite SQL **000-042 + inverses** verte ; pytest complet
  **266/266** (venv monté).
- **Score** : C7 **60 % → 65 %**. Moyenne indicative ≈ 75 %.

---

## Confirmations à obtenir du propriétaire

- **C7 — plafond de vraisemblance** : la valeur **10 000** (unités par ligne
  de comptage) est une **valeur par défaut posée par l'équipe de
  développement (2026-09-25)**, pas une décision du propriétaire. À
  confirmer, modifier ou remettre `a_definir` par le propriétaire.

---

### Cycle 52 — C13 : chasse aux trous de couverture — 2026-09-25

- **Diagnostic** (liste validée par le propriétaire) : `verifier-stock-reel.mjs`
  était obsolète (casse/retour client à un seul temps, « pas de bouton Casse
  pour l'agent stock ») — suite rouge invisible car la CI n'exécute que
  pytest ; `declarations.html`, `/exploitation/derniere-sauvegarde`,
  `/configuration/logo` (depuis l'écran), `/tableau-bord/articles-offerts-en-attente`
  et `/ventes/synthese-jour` n'étaient traversés par aucune suite.
- **Livré** : `verifier-stock-reel.mjs` RÉÉCRIT — déclarations sans effet
  stock, validation sur `declarations.html` (casse décrémente, retour
  revendable réintègre, article offert décrémente), agent stock avec bouton
  Casse de déclaration — **38/38** ; nouveau `verifier-exploitation-reel.mjs`
  — voyant sauvegarde via la vraie route, cartes Ventes du jour / Articles
  offerts, logo téléversé/affiché/supprimé depuis l'écran, 403 agent stock
  — **12/12** ; ajouté à `verifier_tout.sh` et `package.json` ;
  `verifier-routes-appelees.mjs` **55/55**.
- **Score** : C13 **63 % → 68 %**. Moyenne indicative ≈ 75 %.

---

### Cycle 53 — C0 : exécutable reconstruit avec la pile à jour — 2026-09-25

- **Diagnostic** : la ligne C0 annonçait encore « Reste : mode kiosque,
  lockfile » alors que les deux étaient livrés au cycle 46 ; l'exécutable
  distribuable était resté sur l'ancienne pile (FastAPI 0.117.1) et
  l'ancien lanceur (sans kiosque) ; `server/.venv` n'avait pas été monté
  (la montée C11 avait été validée dans un venv séparé, le point f étant
  alors en cours).
- **Livré** : `server/.venv` monté sur le lock haché (FastAPI 0.141.1 /
  starlette 1.7.0 / pytest 9.1.1) ; `Akuma.exe` reconstruit (**28,8 Mo**)
  ; trace mise à jour dans `server/fabrication/DERNIER_RESULTAT.md`
  (historique des 3 constructions) ; ligne C0 corrigée (cycles 46 et 53).
- **Preuves par exécution réelle** : exe lancé depuis `dist/` (base dédiée
  `quincaillerie_c0`) — `/sante` ok, journal HTTP : `/app/connexion.html`,
  `theme.css`, `styles.css`, `akuma-logo.png`, `api.js`, `favicon.ico`
  tous en **200** ; processus Edge vérifié :
  `msedge.exe --kiosk http://127.0.0.1:49322/app/connexion.html ...`.
- **Score** : C0 **70 % → 78 %**. Moyenne indicative ≈ 76 %.
- **Reste (C0)** : aucun point ouvert identifié.
...
- **Décision du propriétaire** : le rapprochement à l'import du stock initial
  doit être **insensible aux accents et à la casse**, et traiter aussi les
  **espaces multiples et les tirets** (« Fer à béton 12 » == « Fer à
  béton  12 » == « Fer à béton-12 ») ; les paires douteuses sont présentées
  au responsable, jamais tranchées d'office. Rattaché à **C1** (intégrité).
- **Livré** : migration **046** — `normaliser_nom_article()` (minuscules,
  `translate` des accents, tirets/tirets bas/apostrophes → espace, espaces
  multiples réduits, `btrim`) ; `db/outils/importer_stock_initial.py`
  rapproche par nom normalisé, refuse de choisir entre plusieurs fiches et
  imprime une section « **À TRANCHER par le responsable** » (ligne ignorée,
  bilan comptabilisé).
- **Preuves par exécution réelle** (base dédiée `quincaillerie_c16`) :
  suite SQL **000-046 + inverses** verte (7 contrôles 046) ; import réel
  d'un Excel : « Fer a beton 8 mm », « Fer à béton  8 mm » et « Fer à
  béton-8 mm » → **une seule fiche réutilisée** (stocks 48/2, aucun
  doublon), « Tuyau PVC »/« tuyau pvc » → **À TRANCHER** (ligne ignorée),
  rejeu identique → « 0 chargée, 4 déjà présente(s) » (idempotence).
- **Score** : C1 **90 % → 92 %**. Moyenne indicative ≈ 76 %.
