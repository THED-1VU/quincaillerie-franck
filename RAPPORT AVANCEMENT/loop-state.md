...
- **Décision du propriétaire** : plafond = **10 000** unités par ligne de
  comptage (la mécanique de la migration 032 s'active).
- **Livré** : migration **040** (+ inverse) — `plafond_vraisemblance_comptage
  = 10000`, `a_decider = FALSE`, référence de décision consignée ; jeu
  d'essai synchronisé ; tests pytest réécrits (10 001 refusé / 10 000
  accepté / paramètre re-décidable) ; 2 contrôles SQL ajoutés (section 13).
- **Preuves** : suite SQL **000-040 + inverses** verte ; pytest complet
  **266/266** (venv monté).
